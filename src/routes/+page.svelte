...
<div class="homeParentContainer commonContainerStyle">
    <div class="homeContainer">
        ...
        <div>
            <h1>Nostree</h1>
            <p>
                A Nostr-based application to create, manage and discover link lists, show notes and
                other stuff.
            </p>
        </div>
        {#if !$ndkUser}
            <Login mode="primary" />
        {:else}
            <Button on:click={() => goto(`/${$ndkUser?.npub}`)} mode="primary" isBlock isRounded
                >Profile</Button
            >
            <Button on:click={() => goto('/new')} mode="primary" isBlock isRounded
                >Manage lists</Button
            >
        {/if}
    ...
    <p>
        ...
        <button
            on:click={() =>
                goto('/npub1gzuushllat7pet0ccv9yuhygvc8ldeyhrgxuwg744dn5khnpk3gs3ea5ds')}
