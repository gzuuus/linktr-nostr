<script lang="ts">
    ...
    function showEvents() {
        if ($ndkUser) {
            ...
            $ndk.fetchEvents({ kinds: [30303 as number], authors: [userPubDecoded] }).then(
                (fetchedEvent) => {
    ...
    <div>
        {#if $ndkUser}
            <div class:hidden={showCreateNewList}>
                ...
                <button class="isBlock" on:click={() => (showCreateNewList = !showCreateNewList)}
                    >Create new list
                </button>
            </div>
            <div class:hidden={!showCreateNewList}>
                                    ...
                                    <div class="alertContainer">
                                        <h3>
                                            🔺 List in the old format <span class="inline-span"
                                                ><InfoDialog
                                                    whatInfo="list-old-format-migrate"
                                                /></span
                                            >
                                        </h3>
