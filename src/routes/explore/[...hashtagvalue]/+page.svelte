...
<svelte:head>
 ...
 <!-- <script lang="ts">
  // TODO!!!!
  import ndk from "$lib/stores/provider";
  import { unixToDate, findOtherTags, naddrEncodeATags, processHashtags, filterDbEvents, sortEventList } from "$lib/utils/helpers";
  import type { NDKEvent, NDKFilter} from "@nostr-dev-kit/ndk";
  import ProfileCardCompact from "$lib/components/profile-card-compact.svelte";
  import ExploreIcon from "$lib/elements/icons/explore-icon.svelte";
  import { kindLinks } from "$lib/utils/constants";
  ...
  let showSearchBar: boolean = false;
  let exploreResults: NDKEventStore<ExtendedBaseType<NDKEvent>>
  let exploreCachedResults: NDKEvent[] | undefined
  let exploreNetwork: boolean = false;
  $: {
    let hashtag = $page.params.hashtagvalue;
    let authors = $localStore.currentUserFollows;
    ...
    if (browser) fetchDbEvents(ndkFilter);

  }
  async function fetchDbEvents(filter:NDKFilter){
    exploreCachedResults = await filterDbEvents(filter)
    console.log(exploreCachedResults);
    if (!exploreCachedResults?.length){
      fetchEvents(filter).then(() => {
      exploreResults?.startSubscription();
      isSubscribe = true;
    });
  } else {
    sortEventList(exploreCachedResults)
  }
  }
...
<svelte:head>
  ...
  {#each exploreCachedResults ?? $exploreResults ?? [] as event}
    <div class="common-container-content">
      <ProfileCardCompact userPub={event.author.npub} />
      <div>
        <h3>{event.tagValue("title")}</h3>
