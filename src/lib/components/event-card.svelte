<script lang="ts">
  ...
  import { unixToDate, buildEventPointer, findListTags, sortEventList, findOtherTags, sharePage, naddrEncodeATags } from "$lib/utils/helpers";
      ...
      <div class="eventContainerButtons">
        <div class:full-width={eventList.length > 1} class:space-between={eventList.length > 1}> 
          ...
          >
            ...
            <code class="inline-span" style="padding-right: 0.5em;">{currentIndex + 1}</code>
          </button>
        ...
        {#if label !== 'nostree' }
          <div  >
            <div class:hidden={!showListsIndex} class="no-line-height listLinkOutContainer listLinkOutContainerContent">
              {#if $ndkUser}
    ...
    {#each eventList as event}
      <div class="eventContainer" >
        ...
        <div class="infoBox">
            ...
            <a href="https://nostr.com/{buildEventPointer(event.id, [event.relay?.url ?? ''], event.pubkey, event.kind, event.tagValue('d'))}" target="_blank" rel="noreferrer"><button class="infoButton"><LinktOut size={20} color=var(--accent-color)/></button></a>
          </div>
      </div>
    {/each}
{/if}
